...
ECHO         ****************************************
npx nativefier -e %electron% -u %user_agent% -n "%app_name%" "%website_url%" --ignore-gpu-blacklist --single-instance --tray %enable_tray% --enable-es3-apis --verbose --background-color "%background_color%" --internal-urls %internal_urls% --icon %ico_file% >>%debug% 2>&1
exit /b