...
nativefier --verbose -n %NAME% "%WEBSITE_URL%" --tray --enable-es3-apis --single-instance --background-color "%BACKGROUND_COLOR%" --internal-urls "%INTERNAL_URLS%" --global-shortcuts %GLOBAL_SHORTCUTS% --bookmarks-menu %BOOKMARKS_MENU% --user-agent %USER_AGENT% --icon %ICON_PATH% "%BASE_FOLDER%"