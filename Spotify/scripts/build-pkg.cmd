...
nativefier --verbose -n %NAME% %WEBSITE_URL% --tray --enable-es3-apis --widevine --background-color %BACKGROUND_COLOR% --internal-urls %INTERNAL_URLS% --inject %SCRIPT% --inject %CSS% --user-agent %USER_AGENT% --icon %ICON_PATH% %BASE_FOLDER% && python3 -m castlabs_evs.vmp sign-pkg --persistent %SIGN_PATH%