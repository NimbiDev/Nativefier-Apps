...
nativefier --verbose -n %NAME% %WEBSITE% --tray --enable-es3-apis --widevine --background-color %BG_COLOR% --internal-urls %INTERNAL_URLS% --icon %ICO_FILE% --user-agent %USER_AGENT% && copy %UPGRADE_FILE% %UPGRADE_PATH% && copy %PS_FILE% %PS_PATH%